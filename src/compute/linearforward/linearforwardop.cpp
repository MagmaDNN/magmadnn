...
Tensor<T> *LinearForwardOp<T>::_eval(bool recompute) {
    ...
    if (input_tensor->get_memory_type() == HOST) {
        ...
        auto weights_mem = dnnl::memory(this->dnnl_fwd_pdesc_->weights_desc(), this->dnnl_cpu_engine_,
        ...
        if (use_bias) {
            auto bias_mem = dnnl::memory(this->dnnl_fwd_pdesc_->bias_desc(), this->dnnl_cpu_engine_,
    ...
    if (use_bias) {
        if (this->output_tensor->get_memory_type() == HOST) {
            magmadnn::math::bias_add_cpu(this->output_tensor, this->bias_tensor, this->output_tensor);
        }
#if defined(MAGMADNN_HAVE_CUDA)
        else {
            magmadnn::math::bias_add_device(this->get_custream(), this->output_tensor, this->bias_tensor,
                                            this->output_tensor);
            if (!this->get_async()) cudaStreamSynchronize(this->get_custream());
        }
#endif
...
Tensor<T> *LinearForwardOp<T>::_grad(Operation<T> *consumer, Operation<T> *var, Tensor<T> *grad) {
    ...
    if (var == this->input) {
        ...
        if (grad->get_memory_type() == HOST) {
            ...
            dnnl::memory::dims diff_dst_dims = {grad->get_shape(0), grad->get_shape(1)};
            // {out->get_shape(0), out->get_shape(1)};
            ...
            dnnl::memory::desc diff_src_md =
                dnnl::memory::desc(diff_src_dims, dnnl::memory::data_type::f32, diif_src_strides);

            dnnl::memory::desc diff_dst_md =
                dnnl::memory::desc(diff_dst_dims, dnnl::memory::data_type::f32, diff_dst_strides);
            ...
            dnnl::inner_product_backward_data::desc ip_bwd_data_desc =
                dnnl::inner_product_backward_data::desc(diff_src_md, weights_md, diff_dst_md);

            dnnl::inner_product_backward_data::primitive_desc ip_bwd_data_pdesc =
                dnnl::inner_product_backward_data::primitive_desc(ip_bwd_data_desc, this->dnnl_cpu_engine_,
            ...
            auto weights_mem =
                dnnl::memory(weights_md, this->dnnl_cpu_engine_, (void *) this->weights_tensor->get_ptr());

            auto diff_src_mem = dnnl::memory(diff_src_md, this->dnnl_cpu_engine_, (void *) out->get_ptr());
            // (void*)grad->get_ptr());

            auto diff_dst_mem = dnnl::memory(diff_dst_md, this->dnnl_cpu_engine_, (void *) grad->get_ptr());
            // (void*)out->get_ptr());

            dnnl::inner_product_backward_data ip_bwd_data = dnnl::inner_product_backward_data(ip_bwd_data_pdesc);
        ...
        if (grad->get_memory_type() == HOST) {
            ...
            dnnl::memory::desc diff_trans_weights_md =
                dnnl::memory::desc(diff_trans_weights_dims, dnnl::memory::data_type::f32, diff_trans_weights_strides);
            ...
            dnnl::inner_product_backward_weights::desc ip_bwd_weights_desc =
                dnnl::inner_product_backward_weights::desc(src_md,
                                                           // diff_weights_md,
                                                           diff_trans_weights_md, diff_dst_md);

            // Inner product primitive descriptor
            //

            dnnl::inner_product_backward_weights::primitive_desc ip_bwd_weights_pdesc =
                dnnl::inner_product_backward_weights::primitive_desc(ip_bwd_weights_desc, this->dnnl_cpu_engine_,
                                                                     *(this->dnnl_fwd_pdesc_.get()));

            // DNNL memory
            //

            auto src_mem = dnnl::memory(src_md, this->dnnl_cpu_engine_, (void *) this->input_tensor->get_ptr());
            ...
            auto diff_trans_weights_mem =
                dnnl::memory(diff_trans_weights_md, this->dnnl_cpu_engine_, (void *) out->get_ptr());
...
void LinearForwardOp<T>::init_settings() {
    /* setup code in here */
    this->output_shape = {this->input->get_output_shape(0), this->weights->get_output_shape(1)};
...
void LinearForwardOp<T>::init_dnnl_settings() {
    dnnl::memory::dims src_dims = {input->get_output_shape(0), input->get_output_shape(1)};
    dnnl::memory::dims src_strides = {src_dims[1], 1};
    // dnnl::memory::dims src_strides = {1, input->get_output_shape(1)};

    dnnl::memory::dims weights_dims = {weights->get_output_shape(0), weights->get_output_shape(1)};
    dnnl::memory::dims weights_strides = {weights_dims[1], 1};

    dnnl::memory::dims trans_weights_dims = {weights->get_output_shape(1), weights->get_output_shape(0)};
    ...
    dnnl::memory::dims dst_dims = {this->output_shape[0], this->output_shape[1]};
    ...
    auto trans_weights_md = dnnl::memory::desc(trans_weights_dims, dnnl::memory::data_type::f32,
                                               // dnnl::memory::format_tag::ab
                                               // weights_strides
                                               // dnnl::memory::format_tag::any
    ...
    if (use_bias) {
        ...
        dnnl::memory::desc bias_md = dnnl::memory::desc({this->output_shape[1]}, dnnl::memory::data_type::f32, {1});

        inner_product_fwd_desc.reset(new dnnl::inner_product_forward::desc(dnnl::prop_kind::forward_training, src_md,
                                                                           trans_weights_md, bias_md, dst_md));
        ...
        inner_product_fwd_desc.reset(
            new dnnl::inner_product_forward::desc(dnnl::prop_kind::forward_training, src_md, trans_weights_md, dst_md));
    }

    this->dnnl_fwd_pdesc_.reset(
        new dnnl::inner_product_forward::primitive_desc(*(inner_product_fwd_desc.get()), this->dnnl_cpu_engine_));

    this->dnnl_fwd_.reset(new dnnl::inner_product_forward(*(this->dnnl_fwd_pdesc_.get())));
}
#endif
...
void LinearForwardOp<T>::init_bias_settings() {
    if (this->mem_type == HOST) {
        ...
        cudnnErrchk(cudnnGetReductionWorkspaceSize(
            this->get_cudnn_handle(), bias_reduce_settings.descriptor, grad_tmp_descriptor,
