...
class CudaExecContext : public ExecContext {
   public:
    CudaExecContext(int in_devid)
        : devid_(in_devid), custream_(nullptr), cublas_handle_(nullptr), cudnn_handle_(nullptr) {
        cudaError_t cuerr;
    ...
    CudaExecContext() : CudaExecContext(0) {}
    ...
    void devid(int in_devid) { devid_ = in_devid; }

    cublasHandle_t cublas_handle() const { return cublas_handle_; }

    void cublas_handle(cublasHandle_t in_cublas_handle) { cublas_handle_ = in_cublas_handle; }

    cudnnHandle_t cudnn_handle() const { return cudnn_handle_; }

    void cudnn_handle(cudnnHandle_t in_cudnn_handle) { cudnn_handle_ = in_cudnn_handle; }

    cudaStream_t stream() const { return custream_; }

    void stream(cudaStream_t in_stream) { custream_ = in_stream; }
