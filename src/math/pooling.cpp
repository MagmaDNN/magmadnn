...
void pooling_grad_device(Tensor<T> *x, Tensor<T> *y, Tensor<T> *grad, Tensor<T> *out,
    ...
    cudnnErrchk(cudnnPoolingBackward(
        ...
        y->get_cudnn_tensor_descriptor(), y->get_ptr(), grad->get_cudnn_tensor_descriptor(), grad->get_ptr(),
        x->get_cudnn_tensor_descriptor(), x->get_ptr(), &beta, out->get_cudnn_tensor_descriptor(), out->get_ptr()));
}
