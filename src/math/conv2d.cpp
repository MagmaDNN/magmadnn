...
void conv2d_grad_data_device(Tensor<T> *w, Tensor<T> *grad, Tensor<T> *out, conv2d_cudnn_settings settings) {
    ...
    cudnnErrchk( cudnnConvolutionBackwardData(
        ...
        grad->get_ptr(),
        settings.conv_desc,
        settings.bwd_data_algo,
        settings.grad_data_workspace,
        settings.grad_data_workspace_size,
        &beta,
        out->get_cudnn_tensor_descriptor(),
        out->get_ptr())
    );
}
template void conv2d_grad_data_device(Tensor<int> *w, Tensor<int> *grad, Tensor<int> *out, conv2d_cudnn_settings settings);
template void conv2d_grad_data_device(Tensor<float> *w, Tensor<float> *grad, Tensor<float> *out, conv2d_cudnn_settings settings);
template void conv2d_grad_data_device(Tensor<double> *w, Tensor<double> *grad, Tensor<double> *out, conv2d_cudnn_settings settings);
...
void conv2d_grad_filter_device(Tensor<T> *x, Tensor<T> *grad, Tensor<T> *out, conv2d_cudnn_settings settings) {
    ...
    cudnnErrchk( cudnnConvolutionBackwardFilter(
        ...
        grad->get_ptr(),
        settings.conv_desc,
        settings.bwd_filter_algo,
        settings.grad_filter_workspace,
        settings.grad_filter_workspace_size,
        &beta,
        settings.filter_desc,
        out->get_ptr())
    );
}
template void conv2d_grad_filter_device(Tensor<int> *x, Tensor<int> *grad, Tensor<int> *out, conv2d_cudnn_settings settings);
template void conv2d_grad_filter_device(Tensor<float> *x, Tensor<float> *grad, Tensor<float> *out, conv2d_cudnn_settings settings);
template void conv2d_grad_filter_device(Tensor<double> *x, Tensor<double> *grad, Tensor<double> *out, conv2d_cudnn_settings settings);
