...
Tensor<T> *DivOp<T>::_eval(bool recompute) {
    ...
    switch (op_type) {
        case internal::TENSOR_DIV_TENSOR:
            internal::tensor_div_tensor_full(a_tensor, b_tensor, this->output_tensor);
            break;
        case internal::SCALAR_DIV_TENSOR:
            a_tensor->get_memory_manager()->sync(true);
            internal::scalar_div_tensor_full(a_tensor->get(0), b_tensor, this->output_tensor);
            break;
        case internal::TENSOR_DIV_SCALAR:
            b_tensor->get_memory_manager()->sync(true);
            internal::tensor_div_scalar_full(a_tensor, b_tensor->get(0), this->output_tensor);
            break;
        default:
