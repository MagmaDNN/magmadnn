...
void pooling_device(Tensor<T> *x, Tensor<T> *out, cudnn_pooling_settings_t settings) {
    ...
    cudnnErrchk( cudnnPoolingForward(
        ::magmadnn::internal::MAGMADNN_SETTINGS->cudnn_handle,
        settings.poolingDesc,
        &alpha,
...
template void pooling_grad_device(Tensor<int> *x, Tensor<int> *y, Tensor<int> *grad, Tensor<int> *out, cudnn_pooling_settings_t settings);
template void pooling_grad_device(Tensor<float> *x, Tensor<float> *y, Tensor<float> *grad, Tensor<float> *out, cudnn_pooling_settings_t settings);
template void pooling_grad_device(Tensor<double> *x, Tensor<double> *y, Tensor<double> *grad, Tensor<double> *out, cudnn_pooling_settings_t settings);
