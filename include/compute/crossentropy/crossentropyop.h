...
/** Returns an operation, which computes the crossentropy between ground_truth and predicted. This must be passed one-hot encoded data.
 * If not one-hot encoded the return values will not be correct or an error may occur.
 * This operation is equivalent to `negative(reducesum(reducesum(product(ground_truth, log(predicted)), axis=1), axis=0))
