...
template <typename T>
Conv2dLayer<T>::Conv2dLayer(op::Operation<T> *input, const std::vector<unsigned int>& filter_shape, int out_channels, const std::vector<unsigned int>& padding,
    ...
    : Layer<T>::Layer(input->get_output_shape(), input), out_channels(out_channels), use_cross_correlation(use_cross_correlation), use_bias(use_bias),
    filter_initializer(filter_initializer), bias_initializer(bias_initializer) {
...
void Conv2dLayer<T>::init(const std::vector<unsigned int>& filter_shape) {
    ...
    this->filter_tensor = new Tensor<T>({static_cast<unsigned int>(this->out_channels), static_cast<unsigned int>(this->in_channels), filter_shape[0], filter_shape[1]}, 
    ...
    if (use_bias) {
        ...
        this->output = op::conv2dforward(this->input, filter, pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w, use_cross_correlation);
    }
}
...
template <typename T>
Conv2dLayer<T>* conv2d(op::Operation<T> *input, const std::vector<unsigned int>& filter_shape, int out_channels, const std::vector<unsigned int>& padding,
        const std::vector<unsigned int>& strides, const std::vector<unsigned int>& dilation_rates, bool use_cross_correlation, bool use_bias,
        tensor_filler_t<T> filter_initializer, tensor_filler_t<T> bias_initializer) {
    return new Conv2dLayer<T> (input, filter_shape, out_channels, padding, strides, dilation_rates, use_cross_correlation, use_bias, filter_initializer, bias_initializer);
}
template Conv2dLayer<int>* conv2d(op::Operation<int>*, const std::vector<unsigned int>&, int, const std::vector<unsigned int>&, 
    const std::vector<unsigned int>&, const std::vector<unsigned int>&, bool, bool, tensor_filler_t<int>, tensor_filler_t<int>);
template Conv2dLayer<float>* conv2d(op::Operation<float>*, const std::vector<unsigned int>&, int, const std::vector<unsigned int>&,
