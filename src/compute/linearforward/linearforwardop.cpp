...
: Operation<T>::Operation({input, weights}, needs_grad), input(input), weights(weights), copy(copy), use_bias(false) {
    /* setup code in here */
...
template <typename T>
LinearForwardOp<T>::LinearForwardOp(Operation<T> *input, Operation<T> *weights, Operation<T> *bias, bool copy, bool needs_grad)
: Operation<T>::Operation({input, weights, bias}, needs_grad), input(input), weights(weights), bias(bias), copy(copy), use_bias(true) {
    /* setup code in here */
    this->output_shape = {input->get_output_shape(0), weights->get_output_shape(1)};
    this->mem_type = input->get_memory_type();
    this->name = "LinearForward";
...
template <typename T>
LinearForwardOp<T> *linearforward(Operation<T> *input, Operation<T> *weights, Operation<T> *bias, bool copy, bool needs_grad) {
    return new LinearForwardOp<T>(input, weights, bias, copy, needs_grad);
}
template LinearForwardOp<int> *linearforward(Operation<int> *input, Operation<int> *weights, Operation<int> *bias, bool copy, bool needs_grad);
template LinearForwardOp<float> *linearforward(Operation<float> *input, Operation<float> *weights, Operation<float> *bias, bool copy, bool needs_grad);
template LinearForwardOp<double> *linearforward(Operation<double> *input, Operation<double> *weights, Operation<double> *bias, bool copy, bool needs_grad);
