...
void Conv2DForwardOp<T>::onednn_init_settings() {
    ...
    auto weights_md = dnnl::memory::desc(weights_dims, dnnl::memory::data_type::f32, dnnl::memory::format_tag::nchw);
    ...
    dnnl::convolution_forward::desc conv_desc = dnnl::convolution_forward::desc(
        dnnl::prop_kind::forward_training, dnnl::algorithm::convolution_direct, src_md, weights_md, bias_md, dst_md,
        conv_strides_dims, conv_dilation_dims, conv_padding_dims, conv_padding_dims);
    ...
    this->dnnl_fwd_pdesc_.reset(new dnnl::convolution_forward::primitive_desc(conv_desc, this->dnnl_cpu_engine_));

    this->dnnl_fwd_.reset(new dnnl::convolution_forward(*(this->dnnl_fwd_pdesc_.get())));
}
#endif
