...
void bias_add_device(Tensor<T> *x, Tensor<T> *bias, Tensor<T> *out) {
   ...
   const auto size = x_rows * x_cols;
   const auto grid_dim = ceildiv(size, BLK_SIZE);
      
   kernel_bias_add_device
      <<<grid_dim, BLK_SIZE>>>
      (x->get_ptr(), bias->get_ptr(), out->get_ptr(), x_rows, x_cols);
}
...
template <typename T>
void bias_add_device(
      cudaStream_t custream, Tensor<T> *x, Tensor<T> *bias, Tensor<T> *out) {
...
template void bias_add_device(cudaStream_t custream, Tensor<int> *x, Tensor<int> *bias, Tensor<int> *out);
template void bias_add_device(cudaStream_t custream, Tensor<float> *x, Tensor<float> *bias, Tensor<float> *out);
template void bias_add_device(cudaStream_t custream, Tensor<double> *x, Tensor<double> *bias, Tensor<double> *out);
