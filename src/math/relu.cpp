...
void relu_device(Tensor<T> *x, Tensor<T> *out, relu_cudnn_settings_t settings) {
    ...
    cudnnErrchk(cudnnActivationForward(::magmadnn::internal::MAGMADNN_SETTINGS->cudnn_handle, settings.descriptor,
                                       &alpha, x->get_cudnn_tensor_descriptor(), x->get_ptr(), &beta,
...
void relu_grad_device(Tensor<T> *x, Tensor<T> *relu_out, Tensor<T> *grad, Tensor<T> *out,
    ...
    cudnnErrchk(cudnnActivationBackward(::magmadnn::internal::MAGMADNN_SETTINGS->cudnn_handle, settings.descriptor,
                                        &alpha, relu_out->get_cudnn_tensor_descriptor(), relu_out->get_ptr(),
                                        grad->get_cudnn_tensor_descriptor(), grad->get_ptr(),
                                        x->get_cudnn_tensor_descriptor(), x->get_ptr(), &beta,
                                        out->get_cudnn_tensor_descriptor(), out->get_ptr()));
}
template void relu_grad_device(Tensor<int> *x, Tensor<int> *relu_out, Tensor<int> *grad, Tensor<int> *out,
                               relu_cudnn_settings_t settings);
template void relu_grad_device(Tensor<float> *x, Tensor<float> *relu_out, Tensor<float> *grad, Tensor<float> *out,
                               relu_cudnn_settings_t settings);
template void relu_grad_device(Tensor<double> *x, Tensor<double> *relu_out, Tensor<double> *grad, Tensor<double> *out,
                               relu_cudnn_settings_t settings);
#endif
