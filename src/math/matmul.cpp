...
template <> void matmul(float alpha, bool trans_A, Tensor<float> *A, bool trans_B, Tensor<float> *B, float beta, Tensor<float> *C) {
	...
	if (A->get_memory_type() == HOST) {
		...
		cblas_sgemm(CblasRowMajor, a_trans, b_trans,
			M, N, K,
			alpha, A->get_ptr(), ldda,
			B->get_ptr(), lddb,
			beta, C->get_ptr(), lddc);
	}
		...
		MAGMA_SGEMM_ROWMAJOR(A->get_ptr(), B->get_ptr(), C->get_ptr(),
			M, N, K,
			alpha, beta, a_trans, b_trans, ldda, lddb, lddc);
	}
	#endif
}
...
template <> void matmul(double alpha, bool trans_A, Tensor<double> *A, bool trans_B, Tensor<double> *B, double beta, Tensor<double> *C) {
	...
	if (A->get_memory_type() == HOST) {
		...
		MAGMA_DGEMM_ROWMAJOR(A->get_ptr(), B->get_ptr(), C->get_ptr(),
			M, N, K,
			alpha, beta, a_trans, b_trans, ldda, lddb, lddc);
	}
	#endif
